function count(str) {
    ...
    function convertTest(val1, val2, test) {
        switch (test) {
            case '<':
                // console.log('rrr')
                return val1 < val2;
    ...
    function apply(target, val, incOrDec) {
        // console.log([getVal(target)+val]);
        let response = 0;
        switch (incOrDec) {
            case 'inc':
                // console.log('inc');
                response = (getVal(target) + val);
                break;
            case 'dec':
                // console.log([getVal(target)]);
                response = (getVal(target) - val);
                break;
            default:
                break
    ...
    function getVal(val) {
        // console.log('val : ' + (dict[val] || 0))
        return dict[val] || 0;
    }

    function getcurrentMax(dict) {
        let max = 0;
        for (let i in dict) {
            if (dict[i] > max) {
                max = dict[i]
    ...
    for (let i in str) {
        let line = str[i];
        // console.log([line[4],line[6],line[5]]);
        if (convertTest(getVal(line[4]), line[6], line[5])) {
            // console.log(line)
            // console.log([getVal(line[4]),line[6],line[5]]);
            // console.log(apply(line[0],line[2],line[1]))
            dict[line[0]] = apply(line[0], line[2], line[1])
        }
        let tempMax = getcurrentMax(dict);
        if (tempMax > max) {
            max = tempMax;
        }
    }
    return [getcurrentMax(dict), max];

}

let inst = 'b inc 5 if a > 1\n' +
    'a inc 1 if b < 5\n' +
    'c dec -10 if a >= 1\n' +
    'c inc -20 if c == 10';
let real = 'v inc 523 if t == 6\n' +
